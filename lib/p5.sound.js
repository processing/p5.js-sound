...
eq = function () {
  ...
  p5.EQ.prototype.modBand = function (band, vol, freq) {
    if (vol != null) {
      this.bands[band].gain.value = vol;
    }
    if (freq != null) {
      this.bands[band].frequency.value = freq;
    }
  };
  /**
...
src_app = function () {
  ...
  var p5SOUND = sndcore;
  return p5SOUND;
}(sndcore, master, helpers, errorHandler, panner, soundfile, amplitude, fft, signal, oscillator, env, pulse, noise, audioin, filter, eq, delay, reverb, metro, looper, soundRecorder, peakdetect, gain, distortion);
}));