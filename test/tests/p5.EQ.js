...
describe('p5.EQ', function () {
  ...
  describe('methods', function () {
    ...
    it('can set freq and gain to all bands', function (done) {
      ...
      setTimeout(() => {
        expect(eq.bands[0].biquad.frequency.value).to.be.approximately(1000, 1);
        expect(eq.bands[0].biquad.gain.value).to.be.approximately(10, 1);
        expect(eq.bands[1].biquad.frequency.value).to.be.approximately(2000, 1);
        expect(eq.bands[1].biquad.gain.value).to.be.approximately(20, 1);
        expect(eq.bands[2].biquad.frequency.value).to.be.approximately(3000, 1);
